...
const LookAhead: FC = () => {
    const [expanded, setExpanded] = useState(`panel-${yearNumber}-${monthNumber}-${todayDateNumber}`)
        ...
        >
            {months.map((month, monthIdx) => {
                return month.days.map((day, dayIdx) => {
                    const prompts = orderedPrompts?.[month.yearNumber]?.[month.monthNumber]?.[day] || []
                    ...
                    return (
                        <Accordion
                            ...
                            sx={{
                                boxShadow: 'inset -1px -10px 13px -8px rgba(0,0,0,0.35)',
                                '&.Mui-expanded': {
                                    margin: 0,
                                },
                            }}
                        >
                                        ...
                                        ? (
                                            <Box
                                                ...
                                                sx={(theme) => ({
                                                    ...
                                                    bottom: '-12px',
                                                    margin: '0 auto',
                                                    boxShadow: 'inset -1px -10px 13px -8px rgba(0,0,0,0.55)',
                                                })}
                                            />
                                        )
                                        : null
                            ...
                            <AccordionDetails
                                sx={{
                                    ...
                                    minHeight: monthIdx === 0 && dayIdx === 0 ? '40vh' : 'auto',
                                }}>
                                {prompts.map((prompt, promptIdx) => <Prompt key={promptIdx} prompt={prompt} />)}
                                <Button
                                    sx={{
                                        // width: '100px'
                                    }}
